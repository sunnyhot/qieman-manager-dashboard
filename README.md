...
App 通过 `raw.githubusercontent.com` 读取 `releases/macos/latest.json` 检查更新。只创建 Release 或只上传 zip 都不够；必须等 Actions 回写的 `latest.json` 进入 `main` 后，App 才能检查到新版本。
...
```bash
curl -fsSL "https://raw.githubusercontent.com/sunnyhot/qieman-manager-dashboard/main/releases/macos/latest.json"
