...
**当前线上版本**: v3.0.2（GitHub Release + `releases/macos/latest.json`）
...
```bash
# 构建 macOS App
APP_VERSION=3.0.2 bash scripts/build_macos_app.sh  # → dist/macos-app/QiemanDashboard.app
...
1. **dashboard/ 服务包较大** — 路由、HTML 渲染、平台数据和快照归一化已从入口拆出，但仍需按模块精确定位
...
- Python 本地服务修改优先定位 `dashboard/` 包；`dashboard_server.py` 只是入口，无 pip 依赖
...
- 发布流程：提交功能代码 → 打 tag（如 `v3.0.2`）→ 推送 `main` 和 tag → GitHub Actions 构建 zip、创建 Release、回写 `releases/macos/latest.json`
