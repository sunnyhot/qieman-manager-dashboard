...
1. **OverviewSectionView.swift 较大 (1246 行)** — 总览、简报和状态面板集中，修改需避免顺手重排无关 UI
...
6. **CLI JSON 契约** — Agent 消费 snake_case 字段，修改需补契约测试
7. **且慢 API 非公开** — 随时可能变更需维护
8. **计划模拟不等于真实回测** — 当前 `PlanSimulationSummary` 不拉历史净值，只模拟未来计划投入

## Agent 工作指南

- 修改 UI 时注意涨跌颜色用 AppPalette（红涨绿跌）
- CLI 命令修改优先定位 `Core/QiemanCommandLine.swift`，保持 snake_case JSON 兼容
...
- 发布流程：提交功能代码 → 打 tag（如 `v3.2.0`）→ 推送 `main` 和 tag → GitHub Actions 构建 zip、创建 Release、回写 `releases/macos/latest.json`
