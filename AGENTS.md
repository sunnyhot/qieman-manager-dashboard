...
**当前发布版本**: v3.3.0（GitHub Release + `releases/macos/latest.json`）
...
| 文件 | 行数 | 职责 |
|---|---|---|
| `Core/Models/` | 1841 (9 文件) | 数据模型：基金、持仓、净值、交易记录等。按域拆为 AppEnums/Query/ManagerWatchSettings/SnapshotPayloads/PlatformPayloads/PersonalAsset/UserPortfolio/PersonalTrade/PersonalPlan |
...
| `Core/CLI/DTOs.swift` | 323 | 19 个命令的输出 DTO（与 App 模型 Codable 隔离） |
...
| `Views/Platform/` | 1193 (7 文件) | 平台子视图：ForumRows / PlatformActionRow / StrategyRadarPanel / PlatformActionDetailCard / HoldingCard / PlatformHoldingsPieChart / PlatformMonthlyOverview |
...
- CLI 命令修改：DTO 在 `Core/CLI/DTOs.swift`，路由/handler 在 `Core/QiemanCommandLine.swift`；新增命令需同步：① 加 DTO ② 加 case + handler ③ 加 `CLIContractSnapshotTests` 快照 ④ 若新增 Swift 文件需更新 `scripts/build_qieman_cli.sh`
