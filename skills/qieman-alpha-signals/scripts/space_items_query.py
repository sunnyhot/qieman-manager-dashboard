...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="查询个人空间动态（space-items）")
    parser.add_argument("--space-user-id", default="", help="spaceUserId")
    parser.add_argument("--user-name", default="", help="用于反查 spaceUserId")
    parser.add_argument("--broker-user-id", default="", help="用于反查 spaceUserId")
...
def resolve_space_user_id(args: argparse.Namespace, qcs, client) -> str:
    if args.space_user_id.strip():
        return args.space_user_id.strip()
    if not (args.user_name.strip() or args.broker_user_id.strip()):
        raise SystemExit("请提供 --space-user-id，或提供 --user-name/--broker-user-id 供反查")

    qcs.ensure_auth_available(client, "space-items")
    users = qcs.fetch_following_users(
        client=client,
        page_size=max(50, args.page_size),
        pages=max(5, args.pages),
    )
    target_name = args.user_name.strip().lower()
    target_broker = args.broker_user_id.strip()
    for user in users:
        if target_broker and user.broker_user_id == target_broker:
            return user.space_user_id
        if target_name and target_name in user.user_name.lower():
            return user.space_user_id
    raise SystemExit("没有找到匹配的 spaceUserId，请先调用 following_users_query.py 确认列表")
...
def run(args: argparse.Namespace) -> int:
    ...
    space_user_id = resolve_space_user_id(args, qcs, client)
    user_info = qcs.fetch_space_user_info(client, space_user_id)
    posts = qcs.fetch_space_items(
        client=client,
    ...
    if args.json:
        ...
        print(f"spaceUserId={space_user_id} | user={user_info.user_name or '未知'} | count={payload['count']}")
        for item in rows:
            print(f"[{item.get('created_at')}] {item.get('title') or '无标题'}")
    return 0
