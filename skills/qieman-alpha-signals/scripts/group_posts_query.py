...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="查询公开小组主理人帖子（group-manager）")
    ...
    parser.add_argument("--prod-code", default="LONG_WIN", help="产品代码，可用于解析 groupId")
    parser.add_argument("--manager-name", default="", help="主理人名称，可用于解析 groupId")
    parser.add_argument("--broker-user-id", default="", help="按发言人 brokerUserId 过滤")
    ...
    parser.add_argument("--all-users", action="store_true", help="不过滤主理人，返回小组全部用户帖子")
...
def run(args: argparse.Namespace) -> int:
    ...
    group_id, source = resolve_group_id(args, qcs, client)
    group = qcs.fetch_group_info(client, group_id, source)
    posts = qcs.fetch_group_posts(
        client=client,
        group=group,
        page_size=max(1, args.page_size),
        pages=max(1, args.pages),
        only_manager=not args.all_users and not args.broker_user_id,
    ...
    if args.json:
        ...
        print(f"groupId={group.group_id} | group={group.group_name or '未知'} | count={payload['count']}")
        for item in rows:
