            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                SectionCard(title: "资产总览", subtitle: "按类型汇总已持有、待确认、定投计划", icon: "rectangle.grid.2x2") {
                    if model.personalAssetRows.isEmpty {
                        EmptySectionState(
                            title: "还没有可展示的个人资产",
                            subtitle: "去「我的持仓」里导入持仓、买入中或定投计划后，这里会自动聚合。",
                            actionTitle: "打开持仓"
                        ) {
                            openPortfolio()
                        }
            ...
            LazyVStack(alignment: .leading, spacing: 16) {
                ...
                SectionCard(title: "买入中", subtitle: "待确认交易单独展示，不并入已成交持仓收益", icon: "clock.badge.exclamationmark") {
                    if let summary = model.pendingTradeSummary, !model.pendingTrades.isEmpty {
                        ...
                        EmptySectionState(
                            title: "还没有买入中记录",
                            subtitle: "可以直接手动录入，或在导入中心上传图片、表格。",
                            actionTitle: "去导入"
                        ) {
                            model.selectedSection = .settings
                        }
                    ...
                    if let summary = model.investmentPlanSummary, !model.investmentPlans.isEmpty {
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            if model.pausedInvestmentPlans.isEmpty && model.endedInvestmentPlans.isEmpty {
                                EmptySectionState(
                                    title: "当前还没有已暂停或已终止的计划明细",
                                    subtitle: "后续把这些计划的截图、表格或手工文本导入到「定投计划」草稿区，并把最后一列状态写成「已暂停」或「已终止」，这里就会自动归档。",
                                    actionTitle: "去导入"
                                ) {
                                    model.selectedSection = .settings
                                }
...
private struct SectionCard<Trailing: View, Content: View>: View {
    let title: String
    let subtitle: String
    let icon: String
...
private struct PersonalAssetTable: View {
    ...
    @State private var pendingUnitAdjustmentRow: PersonalAssetAggregateRow?
    ...
    var body: some View {
            ...
            LazyVStack(spacing: 8) {
                ForEach(rows) { row in
                    PersonalAssetTableRow(
                        ...
                        onUnitAdjust: { mode in
                            pendingUnitAdjustmentRow = row
                            pendingUnitAdjustmentMode = mode
                        }
                    )
                    .equatable()
                }
        ...
        .alert(deleteAlertTitle, isPresented: deleteAlertBinding) {
            Button("删除", role: .destructive) {
                if let row = pendingDeleteRow, let scope = pendingDeleteScope {
                    model.deletePersonalAssetEntry(row, scope: scope)
                }
        ...
        .sheet(item: $pendingUnitAdjustmentMode) { mode in
            if let row = pendingUnitAdjustmentRow {
    ...
    private var deleteAlertBinding: Binding<Bool> {
        Binding(
            ...
            set: { if !$0 { pendingDeleteRow = nil; pendingDeleteScope = nil } }
        )
    }

    private var deleteAlertTitle: String {
        guard let scope = pendingDeleteScope else { return "删除条目" }
        let row = pendingDeleteRow
        switch scope {
        case .holding: return "删除已持有？"
        case .pendingTrades:
            let count = row?.pendingTradeCount ?? 1
            return count > 1 ? "删除买入中 \(count) 条？" : "删除买入中？"
        case .investmentPlans:
            let count = row?.totalPlanCount ?? 1
            return count > 1 ? "删除计划档案 \(count) 条？" : "删除计划档案？"
        case .all: return "删除整条明细？"
        }
    }

    private var deleteAlertMessage: String {
        guard let row = pendingDeleteRow, let scope = pendingDeleteScope else { return "" }
        let itemText = row.fundCode.map { "\(row.fundName)（\($0)）" } ?? row.fundName
        let desc: String
        switch scope {
        case .holding: desc = "已持有记录"
        case .pendingTrades: desc = row.pendingTradeCount > 1 ? "\(row.pendingTradeCount) 条买入中记录" : "买入中记录"
        case .investmentPlans: desc = row.totalPlanCount > 1 ? "\(row.totalPlanCount) 条计划档案" : "计划档案"
        case .all: desc = "所有本地明细记录"
        }
        return "会从本地保存的数据中删除 \(itemText) 的\(desc)。这个操作不会影响且慢账户。"
    }
}

private struct PersonalAssetTableRow: View, Equatable {
    static func == (lhs: PersonalAssetTableRow, rhs: PersonalAssetTableRow) -> Bool {
        lhs.row == rhs.row
    }

    let row: PersonalAssetAggregateRow
    var onDelete: ((PersonalAssetDeleteScope) -> Void)?
    var onUnitAdjust: ((PersonalAssetUnitAdjustmentMode) -> Void)?
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 6) {
                    Text(row.fundName)
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundStyle(AppPalette.ink)
                    Text(row.assetTypeLabel)
                        .font(.system(size: 9, weight: .medium))
                        .foregroundStyle(row.assetType == .stock ? AppPalette.info : AppPalette.brand)
                        .padding(.horizontal, 6)
                        .padding(.vertical, 2)
                        .background((row.assetType == .stock ? AppPalette.info : AppPalette.brand).opacity(0.12), in: Capsule())
                    if let marketLabel = row.rawHolding?.marketLabel ?? row.holdingRow?.holding.marketLabel {
                        Text(marketLabel)
                            .font(.system(size: 9, weight: .medium))
                            ...
                            .padding(.horizontal, 6)
                            .padding(.vertical, 2)
                            .background(AppPalette.info.opacity(0.12), in: Capsule())
                    }
                    Text(row.combinedStatusText)
                        .font(.system(size: 9, weight: .medium))
                        .foregroundStyle(row.hasPending ? AppPalette.warning : AppPalette.brand)
                        .padding(.horizontal, 6)
                        .padding(.vertical, 2)
                        .background((row.hasPending ? AppPalette.warning : AppPalette.brand).opacity(0.12), in: Capsule())
                    if row.hasDrawdownPlan {
                        Text("涨跌幅 \(row.drawdownPlanCount)")
                            .font(.system(size: 9, weight: .medium))
                            .foregroundStyle(AppPalette.info)
                            .padding(.horizontal, 6)
                            .padding(.vertical, 2)
