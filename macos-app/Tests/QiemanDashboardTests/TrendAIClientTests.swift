...
final class TrendAIClientTests: XCTestCase {
    ...
    func testClientSendsOpenAICompatibleRequestAndDecodesReport() async throws {
        ...
        MockTrendURLProtocol.requestHandler = { request in
            ...
            XCTAssertEqual(messages.last?["content"], "user prompt")
        ...
        XCTAssertEqual(decoded.generatedAt, "2026-06-24 10:00:00")
    ...
    func testClientDecodesReportWrappedInMarkdownFenceAndEnvelope() async throws {
        ...
        let decoded = try await client.generateReport(
            prompt: TrendModelPrompt(system: "system", user: "user"),
            settings: providerSettings()
        )

        XCTAssertEqual(decoded.generatedAt, "2026-06-24 10:00:00")
        XCTAssertEqual(decoded.externalSignalStatus, .partial)
    }
    ...
    func testConnectionCheckSendsMinimalPromptAndReturnsPreview() async throws {
        ...
        let result = try await client.checkConnection(settings: providerSettings())
    ...
    func testReasoningOnlyLengthResponseUsesActionableError() async throws {
        ...
        do {
            _ = try await client.checkConnection(settings: providerSettings())
    ...
    func testRateLimitErrorsUseActionableGuidance() {
        let rateLimit = TrendAIClientError.requestFailed(429, "1302 · Rate limit reached for requests")
        XCTAssertTrue(rateLimit.localizedDescription.contains("请求频率"))
        XCTAssertTrue(rateLimit.localizedDescription.contains("稍后重试"))

        let noBalance = TrendAIClientError.requestFailed(429, "1113 · 余额不足或无可用资源包,请充值。")
        XCTAssertTrue(noBalance.localizedDescription.contains("余额不足"))
        XCTAssertTrue(noBalance.localizedDescription.contains("资源包"))
    }
