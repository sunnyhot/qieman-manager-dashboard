    ...
    static func make(
        ...
        let items = selectedRows.map { row in
            ...
            return PersonalAssetComparisonItem(
                ...
                dailyChangeText: dailyChangeCurrencyText(row.estimateChangeAmount, market: row.detectedMarket),
                dailyChangeRateText: dailyChangePercentText(row.estimateChangePct),
